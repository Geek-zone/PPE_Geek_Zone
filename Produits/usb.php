...
try {
	...
	$reponse=$bdd->query('SELECT * FROM produit WHERE categorie="5" ');
	$nb=$reponse->rowCount();
	...
	while($donnees = $reponse ->fetch()){
		echo'<tr>';
		echo'<td class="c2"><img src="../img/usb/'.$donnees['image'].'" height="150" width="150" /></td>';
		echo'<td class="c3" ><a class = "nom">'.$donnees['nom'].'</a></td>';
		echo'<td class="c2">'.$donnees['description'].'</td>';
	
		
		echo'<td class="c3"><a class = "prix">'.$donnees['prix'].'€ </a></td>';
		echo'</tr>';
	}
	echo'</table>';
	$reponse->closeCursor();
